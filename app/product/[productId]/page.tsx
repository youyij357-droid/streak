  ...
  return (
    <main className="min-h-screen bg-[#f6f8fa] text-[#1f2328]">
      <section className="mx-auto flex w-full max-w-6xl flex-col px-5 py-6 sm:px-8 lg:px-10">
        <header className="flex items-center justify-between border-b border-[#d8dee4] pb-5">
        ...
        <section className="grid gap-8 py-10 lg:grid-cols-[1fr_380px] lg:items-start">
          <article className="rounded-lg border border-[#d8dee4] bg-white p-6">
            ...
            <p className="mt-5 max-w-2xl whitespace-pre-wrap text-base leading-7 text-[#59636e]">
              {product.description || "商品の内容を確認してから購入手続きへ進んでください。"}
            </p>
          </article>

          <aside className="rounded-lg border border-[#d8dee4] bg-white p-6 shadow-sm">
            <p className="text-sm font-semibold text-[#656d76]">価格</p>
            ...
            <dl className="mt-5 grid gap-3 border-t border-[#edf0f2] pt-5 text-sm">
              <div className="flex justify-between gap-4">
                <dt className="text-[#656d76]">販売者</dt>
                <dd className="text-right font-medium">{product.shops?.name ?? "STREAK販売者"}</dd>
              </div>
              <div className="flex justify-between gap-4">
                <dt className="text-[#656d76]">決済方法</dt>
                <dd className="text-right font-medium">{paymentNetwork.label}</dd>
              </div>
              <div className="flex justify-between gap-4">
                <dt className="text-[#656d76]">換算レート</dt>
                <dd className="text-right font-medium">1 USDC = {formatJpy(jpyPerUsdc)}</dd>
              </div>
            </dl>
            <Link
              className="mt-6 inline-flex h-12 w-full items-center justify-center rounded-md bg-[#1f2328] px-5 text-sm font-semibold text-white"
              href={`/pay/${product.id}`}
            >
              購入手続きへ進む
            </Link>
