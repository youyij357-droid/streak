  ...
  useEffect(() => {
    ...
    const loadShop = async () => {
      ...
      const { data } = await supabase
        .from('shops')
        .select('shop_name, wallet_address, email, crossmint_api_key')
        .eq('id', sid)
        .single<ShopData>();
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch('/api/shops/settings', {
        ...
        body: JSON.stringify({ shopId, email, crossmintApiKey: crossmintKey }),
      });

      if (!res.ok) throw new Error('save failed');

      localStorage.setItem('streak-notify-payments', notifyPayments.toString());
      setShop(prev => prev ? { ...prev, email, crossmint_api_key: crossmintKey } : prev);
      showToast('Saved!');
    } catch {
